...
#helper to find all zeroes on sudoku board
def find_zero(board):
    result = []
    for row in ROW:
        for col in COL:
...
#find current row and all values in it
...
#find current column and all values in it
...
#find current block and all values in it
...
def mrv(board, empty):
    values = []
    # Generate the values list which includes the index and the domain
    for index in empty:
        values.append((index, candidates(board, index)))

    # Sort by the number of candidates, then by the heuristic value
    ...
    # Return the index with the smallest domain
...
def candidates(board, index):
    # Get the current row, column, or block
...
#helper to find neighbors who are also empty
...
#helper to decode the dictioanry key to row index
...
def backtracking(board):
    ...
    #calculate minimum remaining values
...
if __name__ == '__main__':
    ...
    if len(sys.argv) > 1:
        ...
        if times:
            ...
            print(f"Total puzzles solved: {solved_puzzles}/{len(times)}")
